...
NR = 100; %Number of TR in the sequence
excName = repmat({'bp'},1,NR);
excFA = 30*ones(1,NR); %Designed flip angle, unit: degree
...
dePhase = 2*pi*ones(1,NR); %Dephase related to net gradient within one TR
...
Variable.pool(1).T1 = 1; %T1,unit: s
Variable.pool(1).T2 = 0.05; %T2,unit: s
Variable.pool(1).M0 = 1; %equilibrium magnetization, a.u.
Variable.pool(1).OffRes = -100:5:100; %apparent off resonance in one voxel, relative to rotating frame, unit: Hz
...
Variable.wVOffRes = ones(1,Variable.nSpins); %the probability of each off res value 
...
dummy = 0;
tic
Dictionary = func_dictionary_generation(Sequence, Variable, b1Scale, gamma, b0, dummy, nSubDic, nDic);
toc

%%
figure(1)
hold on
MAll = squeeze(cat(4,Dictionary.Entry));
plot(Variable.pool(1).OffRes,squeeze(abs(MAll(1,end,:)+1i*MAll(2,end,:))),'LineWidth',1.5)
...
figure(2)
hold on
plot(squeeze(abs(MAll(1,:,9)+1i*MAll(2,:,9))),'LineWidth',1.5)
plot(squeeze(abs(MAll(1,:,21)+1i*MAll(2,:,21))),'LineWidth',1.5)
xlabel('#TR')
ylabel('Mxy')
set(gca,'FontSize',14,'FontWeight','bold','LineWidth',1.5,'box','on')
legend('Off-res -60Hz','Off-res 0Hz')